...
func (s *Source) Run(ctx context.Context, domain string, session *subscraping.Session) <-chan subscraping.Result {
	...
	go func() {
		...
		for i := 1; i <= 100; i += 10 {
			...
			baseURL := fmt.Sprintf("https://www.baidu.com/s?wd=site%%3A%s%%20-site%%3Awww.%s&pn=%d&oq=site%%3A%s%%20-site%%3Awww.%s&ie=utf-8&rsv_page=1&topic_pn=",
				domain, domain, i, domain, domain)

			resp, err := session.SimpleGet(ctx, baseURL)
			isnotfound := resp != nil && resp.StatusCode == http.StatusNotFound
			if err != nil && !isnotfound {
				results <- subscraping.Result{Source: "baidu", Type: subscraping.Error, Error: err}
			...
			for _, match := range session.Extractor.FindAllString(src, -1) {
				if !strings.Contains(match, "3Awww.") {
					results <- subscraping.Result{Source: "baidu", Type: subscraping.Subdomain, Value: match}
				}
			}
